...
if app.ENABLE_DUEL_STATUS_SYSTEM:
	import uiduelstatus
...
class GameWindow(ui.ScriptWindow):
	def __init__(self, stream):
		...
		if app.ENABLE_DUEL_STATUS_SYSTEM:
			self.wndDuelStatus = uiduelstatus.DuelStatusDialog()
			self._vpActiveVID = 0
	...
	if app.ENABLE_VIEW_TARGET_HP_SP:
		def SetPCTargetBoard(self, vid, name, hpPercentage, spPercentage, iMinHP, iMaxHP, iMinSP, iMaxSP):
			self.targetBoard.Open(vid, name, hpPercentage, spPercentage, iMinHP, iMaxHP, iMinSP, iMaxSP)
	...
	if app.ENABLE_DUEL_STATUS_SYSTEM:
		def OpenDuelStatusDialog(self, vid, hp, max_hp, sp, max_sp):
			if not vid:
				self._vpActiveVID = 0
				self.wndDuelStatus.Hide()
				return

			self._vpActiveVID = vid
			self.wndDuelStatus.UpdateWindow(vid, hp, max_hp, sp, max_sp)
			self.wndDuelStatus.Show()
